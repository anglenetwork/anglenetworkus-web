...
4. Enter the numeric **Tweet ID** from that URL (the digits after `/status/`).
5. Optionally add a **Caption**—an editor note shown below the embed on the site.
...
The tweet ID must be **numeric only**. Non-status URLs and non-numeric IDs are rejected by Studio validation.
...
The Next.js article renderer uses [`react-tweet`](https://github.com/vercel/react-tweet) with the stored `tweetId`. No Twitter/X API credentials are required for display.

### Troubleshooting

| Issue | What to check |
|-------|----------------|
| Embed missing on site | Confirm **Tweet ID** is numeric and matches the ID in **Tweet URL**. |
| Studio validation error | URL must be a `twitter.com` or `x.com` status link; ID must be digits only. |
...
| Wrong tweet shown | Re-check that **Tweet ID** matches the URL. |
