...
export const categorySlugsQuery = defineQuery(`
  *[_type == "category" && defined(slug.current)]{ "slug": slug.current, name }
`);
...
*[
  ...
  "rank": (
    select(title match $term => 8, 0) +
    select(excerpt match $term => 4, 0) +
    select(epigraph match $term => 3, 0) +
    select(pt::text(bodyRich) match $term => 2, 0) +
    select(pt::text(bodyTextOne) match $term => 1, 0) +
    select(pt::text(bodyTextTwo) match $term => 1, 0) +
    select(pt::text(bodyTextThree) match $term => 1, 0) +
    select(pt::text(bodyTextFour) match $term => 1, 0) +
    select(pt::text(bodyTextFive) match $term => 1, 0) +
    select(category->name match $term => 3, 0) +
    select(coalesce(tags[]->title, tags[]->name) match $term => 2, 0) +
    select(author->name match $term => 1, 0) +
    coalesce(priority, 0) * 0.5 +
    select(featured == true => 0.5, 0)
  ),
  ${SEARCH_FIELDS}
} | order(rank desc, publishedAt desc) [0...$limit]
`;
