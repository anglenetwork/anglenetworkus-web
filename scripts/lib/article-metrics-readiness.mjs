...
export async function checkArticleMetricsReadiness(supabase) {
  ...
  async function probeIncrementFunction() {
    ...
    if (msg.includes('article_id') || msg.includes('required') || code === 'P0001') {
      return true;
    }
  ...
  const hasDailyTable = await probeRelation('article_metrics_daily', 'Daily metrics table');
  const hasTotalsTable = await probeRelation('article_metrics_totals', 'Totals table');
  const hasRankingsView = await probeRelation('article_metrics_rankings', 'Rankings view');
