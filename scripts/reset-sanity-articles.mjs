...
function printSummary({ projectId, dataset, docs, confirmed }) {
  ...
  console.log(
    `  found: ${docs.length} article document(s) (${draftCount} draft document(s) included)`,
  );
...
async function deleteInChunks(client, docs) {
  ...
  for (let i = 0; i < docs.length; i += DELETE_CHUNK_SIZE) {
    ...
    console.log(
      `Deleting chunk ${Math.floor(i / DELETE_CHUNK_SIZE) + 1}/${totalChunks} (${chunk.length} docs)...`,
    );
    await tx.commit();
