...
function creditsForUrl(url) {
  ...
  return {
    creditProvider: isPexels ? 'Pexels' : 'Unsplash',
    creditSourceUrl: url,
    creditLicense: isPexels ? 'Pexels License' : 'Unsplash License',
  };
}
...
function toExternalCover(prev, nextUrl) {
  ...
  const alt = (typeof rest.alt === 'string' && rest.alt.trim()) || 'Hero image for this article';
  ...
  return {
    ...
    externalUrl: url,
    alt,
    ...(rest.epigraph ? { epigraph: rest.epigraph } : {}),
...
function toExternalGalleryItem(prev, nextUrl) {
  const url = nextUrl();
  const { image: _img, ...rest } = prev;
  const alt = (typeof rest.alt === 'string' && rest.alt.trim()) || 'Gallery image';
  const cred = creditsForUrl(url);
  return {
    ...rest,
...
function toExternalEditorialImage(block, nextUrl) {
  ...
  const alt = (typeof rest.alt === 'string' && rest.alt.trim()) || 'Inline article photograph';
  const cred = creditsForUrl(url);
  return {
    ...rest,
...
function mapBodyEditorialImages(body, nextUrl) {
  ...
  return body.map((block) => {
    if (block && typeof block === 'object' && block._type === 'editorialImage') {
...
async function main() {
  ...
  console.log(`Found ${docs.length} published-dataset article documents (${ARTICLE_TYPES.join(', ')}).`);
  console.log(`Mode: ${dryRun ? 'DRY RUN (no writes)' : 'APPLY'}. Status: ${keepStatus ? 'unchanged' : 'set to published'}.`);
  ...
  for (const doc of docs) {
    ...
    const cover =
      doc.cover && typeof doc.cover === 'object' && Object.keys(doc.cover).length > 0
    ...
    const editorialCount = Array.isArray(doc.body)
      ...
      : 0;
    const body = Array.isArray(doc.body) ? mapBodyEditorialImages(doc.body, nextUrl) : undefined;
  ...
  if (dryRun) {
    ...
    console.log('Dry run complete. Re-run without --dry-run to commit. Use --verbose to log each document.');
    return;
  }
  ...
  for (let i = 0; i < operations.length; i += BATCH) {
    ...
    try {
      await tx.commit();
      console.log(`Committed batch ${Math.floor(i / BATCH) + 1} (${slice.length} documents).`);
    } catch (err) {
      console.error(`Batch ${Math.floor(i / BATCH) + 1} failed (${slice.length} docs). Retrying one-by-one…`, err.message);
      for (const op of slice) {
        try {
