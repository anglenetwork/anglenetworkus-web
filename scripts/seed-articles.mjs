...
const UNSPLASH_IMAGE_URLS = [
  ...
  'https://images.unsplash.com/photo-1504639725590-34f098438815?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1520607162513-77705c0f91d0?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1504385983642-3d703b875d7e?w=1600&q=80',
  'https://images.unsplash.com/photo-1516327420634-6213cbbefc68?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1487058792275-0ef4cf9428b0?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1639322537228-f710d84631a1?w=1600&q=80',
  'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=1600&q=80',
  'https://images.unsplash.com/photo-1432888622747-4eb9a8f2c293?w=1600&q=80',
  'https://images.unsplash.com/photo-1542831371-29b0f74f9713?w=1600&q=80',
  'https://images.unsplash.com/photo-1607794549669-0fcbf6c5c983?w=1600&q=80',
  'https://images.unsplash.com/photo-1614064641938-3bbee529c9e0?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1558494949-6ad365c9e59e?w=1600&q=80',
  'https://images.unsplash.com/photo-1504868584819-f8e8b4b6d7e3?w=1600&q=80',
  'https://images.unsplash.com/photo-1555949963-f7eeffbad353?w=1600&q=80',
  'https://images.unsplash.com/photo-1518779578993-ec8949d37344?w=1600&q=80',
  'https://images.unsplash.com/photo-1550751827-1bc4b5a6c96c?w=1600&q=80',
  'https://images.unsplash.com/photo-1551434678-b936736834fc?w=1600&q=80',
  ...
  'https://images.unsplash.com/photo-1531482615713-afdafd43342c?w=1600&q=80',
  'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=1600&q=80',
  'https://images.unsplash.com/photo-1516321497487-e288bd19713f?w=1600&q=80',
 ...
 * @param {string} [opts.epigraph]
 * @param {string} [opts.creditProvider]
 * @param {string} [opts.creditAuthor]
 * @param {string} [opts.creditSourceUrl]
 * @param {string} [opts.creditLicense]
 */
...
function parsePositiveIntArg(flagName, fallback) {
  ...
  if (!Number.isFinite(value) || value < 1) {
    throw new Error(`Invalid ${flagName} value. Expected a positive integer, received "${raw}".`);
  }
  return value;
}
...
function buildSeedDefinitions(baseDefinitions, targetCount) {
  ...
  for (let i = 0; i < targetCount; i++) {
    ...
    out.push(cloneDefinitionWithSeedSuffix(baseDefinitions[templateIndex], cycleIndex, templateIndex));
  }
  return out;
}
...
function buildPublicationSchedule(count) {
  ...
  for (let i = 0; i < count; i++) {
    const publishedAt = new Date(base + i * stepMs + i * 37 * 1000).toISOString();
    const updatedAt = new Date(new Date(publishedAt).getTime() + (3 + (i % 9)) * 60 * 1000).toISOString();
...
async function fetchRequiredRefs(client) {
  const authors = await client.fetch(`*[_type == "author"] | order(name asc) { _id, name }`);
  ...
  const categories = await client.fetch(`*[_type == "category"] | order(name asc) { _id, name }`);
  ...
  const tags = await client.fetch(`*[_type == "tag"] | order(title asc) { _id, title }`);
...
async function deleteExistingArticleFamilyDocs(client) {
  ...
  console.log(`  post: ${byType.post}, opinion: ${byType.opinion}, analysis: ${byType.analysis}, sponsored: ${byType.sponsored} (total ${docs.length})`);
...
function buildPostDoc({
  ...
  const body = buildPortableTextBody(bodyParagraphsForSeed('post', postIndex), `post-${postIndex}`);
...
function buildOpinionDoc({
  ...
  const slugCurrent = slugify(title);
  const body = buildPortableTextBody(bodyParagraphsForSeed('opinion', index), `opinion-${index}`);
  return {
    _type: 'opinion',
    title,
    tickerTitle,
...
function buildAnalysisDoc({
  ...
  const slugCurrent = slugify(title);
  const body = buildPortableTextBody(bodyParagraphsForSeed('analysis', index), `analysis-${index}`);
...
function buildSponsoredDoc({
  ...
  const slugCurrent = slugify(title);
  const body = buildPortableTextBody(bodyParagraphsForSeed('sponsored', index), `sponsored-${index}`);
...
const POST_DEFINITIONS = [
  {
    group: 'A',
    title: 'TSMC outlines fab timeline as CHIPS Act awards ripple through supply chains',
  ...
  {
    group: 'A',
    title: 'Cyber agencies flag VPN supply-chain exposure after vendor patch delays',
  ...
  {
    group: 'B',
    title: 'SEC staff previews AI disclosure expectations for quarterly filings',
  ...
  {
    group: 'B',
    title: 'Cloud spend stays resilient as enterprises renew multiyear platform deals',
  ...
  {
    group: 'B',
    title: 'Science advisers urge updated coastal flood benchmarks before hurricane season',
  ...
  {
    group: 'C',
    title: 'Phone makers tighten default encryption after lobbying on lawful access',
  ...
  {
    group: 'C',
    title: 'Reviewers document battery spread on new foldables ahead of holiday push',
  ...
  },
  {
    group: 'D',
    variant: 'breaking',
    title: 'White House sets briefing on infrastructure priorities after bridge assessment',
  ...
  },
  {
    group: 'D',
    variant: 'developing',
    title: 'Senate panel pauses markup on tech antitrust measure amid amendment rush',
  ...
  },
  {
    group: 'D',
    variant: 'plain',
    title: 'Commerce releases early semiconductor import figures for trade monitors',
  ...
  {
    group: 'E',
    title: 'Regulators clarify breach reporting clocks for critical infrastructure operators',
  ...
  {
    group: 'E',
    title: 'AI chip startups hunt runway as valuations reset after a frenetic funding cycle',
  ...
  {
    group: 'E',
    title: 'Sample-return lab prioritizes contamination controls on early asteroid material',
  ...
  {
    group: 'E',
    title: 'Cyber insurers adjust premiums as ransomware claims shift toward midsize firms',
...
const POST_DEFINITIONS_BATCH2 = [
  {
    group: 'A',
    title: 'Intel details packaging roadmap as rivals chase advanced interconnect yields',
  ...
  {
    group: 'A',
    title: 'Ransomware crews probe exposed APIs after vendor discloses legacy auth bug',
  ...
  {
    group: 'B',
    title: 'Treasury guidance clarifies sanctions screening for crypto infrastructure vendors',
  ...
  {
    group: 'B',
    title: 'Retail traders rotate back into megacap tech as rates outlook stabilizes',
  ...
  {
    group: 'B',
    title: 'NOAA revises storm surge projections for Gulf Coast population centers',
  ...
  {
    group: 'C',
    title: 'Apple and Google face new default-browser scrutiny in overseas markets',
  ...
  {
    group: 'C',
    title: 'Drone reviewers highlight wind limits after holiday delivery pilot expansions',
  ...
  },
  {
    group: 'D',
    variant: 'breaking',
    title: 'Breaking: FAA issues narrow waiver for beyond-line-of-sight drone corridor test',
  ...
  },
  {
    group: 'D',
    variant: 'developing',
    title: 'Developing: Grid operator weighs conservation appeals amid heat dome forecasts',
  ...
  },
  {
    group: 'D',
    variant: 'plain',
    title: 'Labor Dept. posts weekly jobless claims inline with economist consensus',
  ...
  {
    group: 'E',
    title: 'Water utilities accelerate sensor deployments after EPA cybersecurity directive',
  ...
  {
    group: 'E',
    title: 'Open-source maintainers debate SBOM depth after npm ecosystem incident',
  ...
  {
    group: 'E',
    title: 'State AGs coordinate privacy inquiries into student data practices in edtech apps',
  ...
  {
    group: 'E',
    title: 'Copper miners hedge output as smelter fees swing with Chinese demand signals',
...
const OPINION_DEFINITIONS = [
  {
    opinionFormat: 'op-ed',
    disclosure: 'The author consults for a nonprofit focused on broadband access; no direct stake in vendors mentioned.',
    title: 'Op-ed: Export controls need clearer guardrails for open-weight AI releases',
  ...
  },
  {
    opinionFormat: 'op-ed',
    disclosure: null,
    title: 'Second view: Antitrust remedies should preserve interoperability for small SaaS vendors',
  ...
  {
    opinionFormat: 'editorial',
    disclosure:
      'This editorial represents the views of the newsroom’s editorial board on public funding priorities.',
    title: 'Editorial: Congress should fund cyber hygiene grants before mandating new rules',
  ...
  {
    opinionFormat: 'column',
    disclosure: null,
    title: 'Column: The gadget cycle is cooling—good for buyers, stressful for retailers',
  ...
  {
    opinionFormat: 'commentary',
    disclosure: null,
    title: 'Commentary: Markets misread cloud growth if they ignore inference power costs',
...
const OPINION_DEFINITIONS_BATCH2 = [
  ...
  },
  {
    opinionFormat: 'op-ed',
    disclosure: null,
    title: 'Op-ed: Liability shields for security researchers need narrow, audited scope',
  ...
  },
  {
    opinionFormat: 'column',
    disclosure: null,
    title: 'Column: The quiet comeback of on-prem inference for regulated workloads',
  ...
  },
  {
    opinionFormat: 'commentary',
    disclosure: null,
    title: 'Commentary: Earnings calls still underplay power-grid risk to AI scaling',
...
const OPINION_DEFINITIONS_ALL = [...OPINION_DEFINITIONS, ...OPINION_DEFINITIONS_BATCH2];

const ANALYSIS_DEFINITIONS = [
  {
    analysisFocus: 'Maps how CHIPS incentives alter capex timing for foundries and equipment vendors.',
    methodologyNote:
  ...
  {
    analysisFocus: 'Explains why VPN compromises cascade faster when identity stores are shared.',
    methodologyNote:
  ...
  {
    analysisFocus: 'Frames disclosure gaps investors should watch in generative AI footnotes.',
    sourcesNote:
  ...
  {
    analysisFocus: 'Compares cloud revenue durability signals across hyperscalers and regional hosts.',
    sourcesNote:
  ...
  {
    analysisFocus: 'Interprets early import data against fab utilization and inventory drawdowns.',
    sourcesNote:
      'Cross-checks trade tables with vendor shipment disclosures and foundry utilization calls.',
    title: 'Analysis: What early chip import data suggests about inventory digestion',
...
const ANALYSIS_DEFINITIONS_BATCH2 = [
  {
    analysisFocus: 'Traces how memory pricing interacts with handset build plans and channel inventory.',
    methodologyNote:
      'Uses rolling 13-week sell-through where available; imputes gaps with industry shipment tables.',
    sourcesNote: null,
    title: 'Analysis: Why DRAM spot prices are a noisy signal for smartphone demand',
  ...
  {
    analysisFocus: 'Unpacks credential-stuffing success rates when MFA is optional on legacy portals.',
    methodologyNote:
      'Synthetic login experiments use public breach corpora; no live accounts were targeted.',
    sourcesNote: null,
    title: 'Analysis: Where credential stuffing still wins without phishing users',
  ...
  {
    analysisFocus: 'Shows how latency arbitrage shows up in colocation and exchange matching debates.',
    sourcesNote:
      'Draws on exchange rule filings, academic measurement studies, and vendor marketing claims.',
    title: 'Analysis: Parsing fairness claims in microwave vs fiber market data feeds',
  ...
  {
    analysisFocus: 'Weighs backlog quality vs headline GPU unit shipments for AI infrastructure.',
    sourcesNote:
  ...
  {
    analysisFocus: 'Connects wildfire smoke models to indoor air sensor adoption in western states.',
    sourcesNote:
      'Combines EPA AirNow archives, state sensor grants, and retailer sell-through anecdotes.',
    title: 'Analysis: Consumer air purifiers spike when smoke forecasts extend past 48 hours',
...
const ANALYSIS_DEFINITIONS_ALL = [...ANALYSIS_DEFINITIONS, ...ANALYSIS_DEFINITIONS_BATCH2];
...
const SPONSORED_DEFINITIONS = [
  {
    ...
    title: 'Sponsored: Why enterprises batch AI inference jobs to tame power spikes',
  ...
  },
  {
    ...
    title: 'Sponsored: FinOps lessons from teams consolidating observability spend',
  ...
  },
  {
    ...
    title: 'Sponsored: Refresh cycles lengthen—how partners support trade-in programs',
  ...
  },
  {
    ...
    title: 'Sponsored: Building governance boards for cross-border data projects',
...
const SPONSORED_DEFINITIONS_BATCH2 = [
  {
    ...
    title: 'Sponsored: Tabletop exercises that actually improve incident comms under pressure',
  ...
  },
  {
    ...
    title: 'Sponsored: Why cohort dashboards beat vanity metrics for subscription retention',
  ...
  },
  {
    ...
    title: 'Sponsored: Creative testing frameworks for short-form video on news adjacencies',
  ...
  },
  {
    ...
    title: 'Sponsored: Hardening OTA pipelines for firmware on industrial edge devices',
  ...
  },
  {
    ...
    title: 'Sponsored: Mapping evidence lockers before your next SOC 2 renewal window',
...
const SPONSORED_DEFINITIONS_ALL = [...SPONSORED_DEFINITIONS, ...SPONSORED_DEFINITIONS_BATCH2];
...
async function run() {
  ...
  const opinionDefinitionsForRun = OPINION_DEFINITIONS_ALL.map((def) => ({ ...def }));

  console.log('\nReference documents loaded:');
  console.log(`  authors: ${authors.length} (${authors.map((a) => a.name).join(', ')})`);
  console.log(`  categories: ${categories.length}`);
  console.log(`  tags: ${tags.length}`);
  console.log(`  target per category (post/analysis/sponsored): ${perCategory}`);
  console.log(`  mode: ${appendMode ? 'append (keep existing)' : 'replace (delete existing first)'}`);
  ...
  for (let i = 0; i < postDefinitionsForRun.length; i++) {
    ...
    const epigraph = `File photo: visuals supporting “${def.tickerTitle}”.`;
  ...
  for (let i = 0; i < opinionDefinitionsForRun.length; i++) {
    ...
    toCreate.push(
      buildOpinionDoc({
        ...
        tickerTitle: def.tickerTitle,
        excerpt: def.excerpt,
        opinionFormat: def.opinionFormat,
        disclosure: def.disclosure,
        authorRef,
        imageUrl,
        alt,
        epigraph: `Illustration for ${def.opinionFormat} piece.`,
        schedule: schedule[docIndex],
        index: i,
      }),
    );
  ...
  for (let i = 0; i < analysisDefinitionsForRun.length; i++) {
    ...
    toCreate.push(
      buildAnalysisDoc({
        ...
        imageUrl,
        alt,
        epigraph: 'Context imagery for data-forward analysis.',
        schedule: schedule[docIndex],
        index: i,
        tagRotation: i * 3 + 1,
      }),
  ...
  for (let i = 0; i < sponsoredDefinitionsForRun.length; i++) {
    ...
    toCreate.push(
      buildSponsoredDoc({
        ...
        imageUrl,
        alt,
        epigraph: `Presented by ${def.sponsorName}.`,
        schedule: schedule[docIndex],
        index: i,
        tagRotation: i * 4 + 2,
      }),
  ...
  for (const doc of toCreate) {
    try {
      ...
      console.error(`Failed to create [${doc._type}] ${doc.title}:`, err.message);
  ...
  console.log(`  total: ${Object.values(createdByType).reduce((a, b) => a + b, 0)}`);
