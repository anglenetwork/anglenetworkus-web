...
// Query: count(*[    _type in ["post", "opinion", "analysis"] &&    status == "published" && defined(publishedAt) && publishedAt <= now() &&    (  searchText match $term ||  title match $term ||  tickerTitle match $term ||  excerpt match $term ||  cover.caption match $term)  ])
export type SearchEditorialCountAllQueryResult = number;

// Source: sanity/lib/queries.ts
// Variable: searchEditorialCountPostQuery
// Query: count(*[    _type == "post" &&    status == "published" && defined(publishedAt) && publishedAt <= now() &&    (  searchText match $term ||  title match $term ||  tickerTitle match $term ||  excerpt match $term ||  cover.caption match $term)  ])
export type SearchEditorialCountPostQueryResult = number;

// Source: sanity/lib/queries.ts
// Variable: searchEditorialCountOpinionQuery
// Query: count(*[    _type == "opinion" &&    status == "published" && defined(publishedAt) && publishedAt <= now() &&    (  searchText match $term ||  title match $term ||  tickerTitle match $term ||  excerpt match $term ||  cover.caption match $term)  ])
export type SearchEditorialCountOpinionQueryResult = number;

// Source: sanity/lib/queries.ts
// Variable: searchEditorialCountAnalysisQuery
// Query: count(*[    _type == "analysis" &&    status == "published" && defined(publishedAt) && publishedAt <= now() &&    (  searchText match $term ||  title match $term ||  tickerTitle match $term ||  excerpt match $term ||  cover.caption match $term)  ])
export type SearchEditorialCountAnalysisQueryResult = number;

// Source: sanity/lib/queries.ts
// Variable: searchEditorialCountSponsoredQuery
// Query: count(*[    _type == "sponsored" &&    status == "published" && defined(publishedAt) && publishedAt <= now() &&    (  searchText match $term ||  title match $term ||  tickerTitle match $term ||  excerpt match $term ||  cover.caption match $term)  ])
